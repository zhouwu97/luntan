...
func TestDeleteRankingToyCommentRootCascadeCleansOrphanRepliesEvenIfRootAlreadyDeleted(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("sqlmock.New() failed: %v", err)
	}
	...
	mock.ExpectBegin()
	// Root comment whose deleted_at is already set
	mock.ExpectQuery(`(?s)SELECT toy_id, COALESCE\(root_id, id\), parent_id, deleted_at FROM ranking_toy_comments WHERE id = \$1 FOR UPDATE`).
		WithArgs("root-already-deleted").
		WillReturnRows(sqlmock.NewRows([]string{"toy_id", "root_id", "parent_id", "deleted_at"}).
			AddRow("toy-1", "root-already-deleted", nil, time.Now()))

	// Soft delete query still executes to clean up any un-deleted child replies
	mock.ExpectExec(regexp.QuoteMeta(`UPDATE ranking_toy_comments SET deleted_at = now(), updated_at = now() WHERE (id = $1 OR root_id = $1) AND deleted_at IS NULL`)).
		WithArgs("root-already-deleted").
		WillReturnResult(sqlmock.NewResult(1, 2))
	...
	req := httptest.NewRequest(http.MethodDelete, "/api/v1/ranking/toy-comments/root-already-deleted", nil)
	req.Header.Set("Authorization", "Bearer admin-token")
	res := httptest.NewRecorder()
	NewHandler(db).ServeHTTP(res, req)
