...
func TestCreatePostWritesRevisionAndIsIdempotentByUserKey(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT to_char(now() AT TIME ZONE 'Asia/Shanghai', 'YYYY-MM-DD')`)).WillReturnRows(sqlmock.NewRows([]string{"to_char"}).AddRow("2026-09-01"))
	...
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT balance_after FROM point_transactions WHERE user_id = $1 AND idempotency_key = $2`)).WithArgs("u1", "daily:2026-09-01:user:u1").WillReturnError(sql.ErrNoRows)
	...
	mock.ExpectExec(regexp.QuoteMeta(`INSERT INTO point_transactions (id, user_id, source, delta, balance_after, reason, idempotency_key) VALUES ($1, $2, $3, $4, $5, $6, $7)`)).WithArgs(sqlmock.AnyArg(), "u1", "post", int64(1), int64(1), "发布帖子", "daily:2026-09-01:user:u1").WillReturnResult(sqlmock.NewResult(1, 1))
...
func TestPostLikeIsIdempotentAndUpdatesCountOnlyAfterNewRelation(t *testing.T) {
	...
	for index, inserted := range []int64{1, 0} {
		...
		if inserted == 1 {
			...
			mock.ExpectQuery(regexp.QuoteMeta(`SELECT balance_after FROM point_transactions WHERE user_id = $1 AND idempotency_key = $2`)).WithArgs("u1", "daily:2026-09-01:user:u1").WillReturnError(sql.ErrNoRows)
			mock.ExpectExec(regexp.QuoteMeta(`UPDATE users SET points_balance = $1, updated_at = now() WHERE id = $2`)).WithArgs(int64(1), "u1").WillReturnResult(sqlmock.NewResult(1, 1))
			mock.ExpectExec(regexp.QuoteMeta(`INSERT INTO point_transactions (id, user_id, source, delta, balance_after, reason, idempotency_key) VALUES ($1, $2, $3, $4, $5, $6, $7)`)).WithArgs(sqlmock.AnyArg(), "u1", "like", int64(1), int64(1), "点赞帖子", "daily:2026-09-01:user:u1").WillReturnResult(sqlmock.NewResult(1, 1))
		}
		mock.ExpectCommit()
		req := httptest.NewRequest(http.MethodPut, "/api/v1/posts/p1/like", nil)
		req.Header.Set("Authorization", "Bearer access-token")
