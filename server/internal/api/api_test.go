...
func TestCreatePostWritesRevisionAndIsIdempotentByUserKey(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT COALESCE(SUM(delta), 0) FROM point_transactions WHERE user_id = $1 AND delta > 0 AND created_at >= date_trunc('day', now() AT TIME ZONE 'Asia/Shanghai') AT TIME ZONE 'Asia/Shanghai'`)).WithArgs("u1").WillReturnRows(sqlmock.NewRows([]string{"coalesce"}).AddRow(0))
	mock.ExpectExec(regexp.QuoteMeta(`UPDATE users SET points_balance = $1, updated_at = now() WHERE id = $2`)).WithArgs(int64(5), "u1").WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec(regexp.QuoteMeta(`INSERT INTO point_transactions (id, user_id, source, delta, balance_after, reason, idempotency_key) VALUES ($1, $2, $3, $4, $5, $6, $7)`)).WithArgs(sqlmock.AnyArg(), "u1", "post", int64(5), int64(5), "发布帖子", sqlmock.AnyArg()).WillReturnResult(sqlmock.NewResult(1, 1))
...
func TestPostLikeIsIdempotentAndUpdatesCountOnlyAfterNewRelation(t *testing.T) {
	...
	for index, inserted := range []int64{1, 0} {
		...
		if inserted == 1 {
			...
			mock.ExpectQuery(regexp.QuoteMeta(`SELECT points_balance FROM users WHERE id = $1 FOR UPDATE`)).WithArgs("u1").WillReturnRows(sqlmock.NewRows([]string{"points_balance"}).AddRow(0))
			mock.ExpectQuery(regexp.QuoteMeta(`SELECT balance_after FROM point_transactions WHERE user_id = $1 AND idempotency_key = $2`)).WithArgs("u1", "post:like:p1:u1").WillReturnError(sql.ErrNoRows)
			mock.ExpectQuery(regexp.QuoteMeta(`SELECT COALESCE(SUM(delta), 0) FROM point_transactions WHERE user_id = $1 AND delta > 0 AND created_at >= date_trunc('day', now() AT TIME ZONE 'Asia/Shanghai') AT TIME ZONE 'Asia/Shanghai'`)).WithArgs("u1").WillReturnRows(sqlmock.NewRows([]string{"coalesce"}).AddRow(0))
			...
			mock.ExpectExec(regexp.QuoteMeta(`INSERT INTO point_transactions (id, user_id, source, delta, balance_after, reason, idempotency_key) VALUES ($1, $2, $3, $4, $5, $6, $7)`)).WithArgs(sqlmock.AnyArg(), "u1", "like", int64(1), int64(1), "点赞帖子", "post:like:p1:u1").WillReturnResult(sqlmock.NewResult(1, 1))
		}
		mock.ExpectCommit()
		req := httptest.NewRequest(http.MethodPut, "/api/v1/posts/p1/like", nil)
		req.Header.Set("Authorization", "Bearer access-token")
