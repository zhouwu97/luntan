...
func TestCreatePostWritesRevisionAndIsIdempotentByUserKey(t *testing.T) {
	...
	// 发帖积分：直接调用 awardPointsTx，不再需要 to_char 查询
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT points_balance FROM users WHERE id = $1 FOR UPDATE`)).WithArgs("u1").WillReturnRows(sqlmock.NewRows([]string{"points_balance"}).AddRow(0))
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT balance_after FROM point_transactions WHERE user_id = $1 AND idempotency_key = $2`)).WithArgs("u1", sqlmock.AnyArg()).WillReturnError(sql.ErrNoRows)
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT COALESCE(SUM(delta), 0) FROM point_transactions WHERE user_id = $1 AND delta > 0 AND source IN ('post', 'like', 'comment') AND created_at >= date_trunc('day', now() AT TIME ZONE 'Asia/Shanghai') AT TIME ZONE 'Asia/Shanghai'`)).WithArgs("u1").WillReturnRows(sqlmock.NewRows([]string{"coalesce"}).AddRow(0))
	mock.ExpectExec(regexp.QuoteMeta(`UPDATE users SET points_balance = $1, updated_at = now() WHERE id = $2`)).WithArgs(int64(10), "u1").WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec(regexp.QuoteMeta(`INSERT INTO point_transactions (id, user_id, source, delta, balance_after, reason, idempotency_key) VALUES ($1, $2, $3, $4, $5, $6, $7)`)).WithArgs(sqlmock.AnyArg(), "u1", "post", int64(10), int64(10), "发布帖子", sqlmock.AnyArg()).WillReturnResult(sqlmock.NewResult(1, 1))
...
func TestStoreProductsOrdersByRedeemedCount(t *testing.T) {
	...
	defer db.Close()
	mock.ExpectQuery(`(?s)SELECT p\.id, p\.name.*COUNT\(o\.id\) FILTER \(WHERE o\.status IN \('approved', 'claimed', 'completed'\)\).*ORDER BY redeemed_count DESC`).
		WithArgs().
		WillReturnRows(sqlmock.NewRows([]string{"id", "name", "description", "emoji", "points", "color", "stock_total", "stock_reserved", "stock_fulfilled", "redeemed_count"}).
			AddRow("p1", "校园徽章", "纪念品", "🏅", int64(120), 16766842, int64(100), int64(0), int64(0), int64(42)))
...
func TestPostLikeIsIdempotentAndUpdatesCountOnlyAfterNewRelation(t *testing.T) {
	...
	for index, inserted := range []int64{1, 0} {
		...
		if inserted == 1 {
			...
			// 新的点赞积分流程：先锁用户 → 幂等检查 → 点赞次数检查 → 全局上限检查 → 发放
			mock.ExpectQuery(regexp.QuoteMeta(`SELECT points_balance FROM users WHERE id = $1 FOR UPDATE`)).WithArgs("u1").WillReturnRows(sqlmock.NewRows([]string{"points_balance"}).AddRow(0))
			mock.ExpectQuery(regexp.QuoteMeta(`SELECT balance_after FROM point_transactions WHERE user_id = $1 AND idempotency_key = $2`)).WithArgs("u1", "like:post:p1:user:u1").WillReturnError(sql.ErrNoRows)
			mock.ExpectQuery(regexp.QuoteMeta(`SELECT COUNT(*) FROM point_transactions WHERE user_id = $1 AND source = 'like' AND delta > 0 AND created_at >= date_trunc('day', now() AT TIME ZONE 'Asia/Shanghai') AT TIME ZONE 'Asia/Shanghai'`)).WithArgs("u1").WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(0))
			mock.ExpectQuery(regexp.QuoteMeta(`SELECT COALESCE(SUM(delta), 0) FROM point_transactions WHERE user_id = $1 AND delta > 0 AND source IN ('post', 'like', 'comment') AND created_at >= date_trunc('day', now() AT TIME ZONE 'Asia/Shanghai') AT TIME ZONE 'Asia/Shanghai'`)).WithArgs("u1").WillReturnRows(sqlmock.NewRows([]string{"coalesce"}).AddRow(0))
			mock.ExpectExec(regexp.QuoteMeta(`UPDATE users SET points_balance = $1, updated_at = now() WHERE id = $2`)).WithArgs(int64(1), "u1").WillReturnResult(sqlmock.NewResult(1, 1))
			mock.ExpectExec(regexp.QuoteMeta(`INSERT INTO point_transactions (id, user_id, source, delta, balance_after, reason, idempotency_key) VALUES ($1, $2, $3, $4, $5, $6, $7)`)).WithArgs(sqlmock.AnyArg(), "u1", "like", int64(1), int64(1), "点赞", "like:post:p1:user:u1").WillReturnResult(sqlmock.NewResult(1, 1))
		}
		mock.ExpectCommit()
		req := httptest.NewRequest(http.MethodPut, "/api/v1/posts/p1/like", nil)
		req.Header.Set("Authorization", "Bearer access-token")
