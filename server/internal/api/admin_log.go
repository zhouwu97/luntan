...
func appendAdminLogTx(ctx context.Context, tx *sql.Tx, operatorID, action, targetType, targetID, reason, requestID string, payload map[string]any, now time.Time) error {
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO admin_logs (id, operator_id, action, target_type, target_id, reason, payload, previous_hash, hash, request_id, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7::jsonb, $8, $9, $10, $11)`, id, operatorID, action, targetType, targetID, reason, data, previousHash, hash, requestID, now); err != nil {
		return err
	}
