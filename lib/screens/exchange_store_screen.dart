...
class ExchangeStoreScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (apiRepository != null) return _ApiExchangeStoreScreen(repository: apiRepository!);
...
class _ApiExchangeStoreScreen extends StatefulWidget {
  ...
  @override
  State<_ApiExchangeStoreScreen> createState() => _ApiExchangeStoreScreenState();
}
...
class _ApiExchangeStoreScreenState extends State<_ApiExchangeStoreScreen> {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<ApiStoreProduct>>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) return Center(child: Text('商店加载失败：${snapshot.error}'));
            ...
            children: [
              FutureBuilder<int>(
                ...
                builder: (context, balance) => Container(
                  ...
                  child: Text('校园周边兑换    ${balance.data ?? '…'} 积分', style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w900)),
                ),
              ),
              const SizedBox(height: 20),
              const Text('全部商品', style: TextStyle(color: AppTheme.textPrimary, fontSize: 18, fontWeight: FontWeight.w800)),
              ...
              GridView.builder(
                ...
                itemBuilder: (_, index) => _ApiProductCard(product: items[index], onRedeem: () => _redeem(items[index])),
              ),
            ],
          );
        },
  ...
  Future<void> _redeem(ApiStoreProduct product) async {
    try {
      await widget.repository.redeem(product.id);
      if (!mounted) return;
      setState(() => balanceFuture = widget.repository.balance());
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('已兑换${product.name}，请留意领取通知')));
    } catch (error) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(userFacingApiMessage(error, fallback: '兑换失败，请稍后重试'))));
    }
  }
}
...
class _ApiProductCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.all(13), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), border: Border.all(color: AppTheme.border)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(child: Container(width: double.infinity, alignment: Alignment.center, decoration: BoxDecoration(color: Color(product.color), borderRadius: BorderRadius.circular(15)), child: Text(product.emoji, style: const TextStyle(fontSize: 52)))), const SizedBox(height: 10), Text(product.name, style: const TextStyle(color: AppTheme.textPrimary, fontSize: 14, fontWeight: FontWeight.w800)), const SizedBox(height: 3), Text(product.description, style: const TextStyle(color: AppTheme.textSecondary, fontSize: 11)), const SizedBox(height: 9), Row(children: [Expanded(child: Text('${product.points} 积分', style: const TextStyle(color: AppTheme.orange, fontSize: 12, fontWeight: FontWeight.w800))), SizedBox(height: 30, child: FilledButton(onPressed: onRedeem, style: FilledButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 10)), child: const Text('兑换', style: TextStyle(fontSize: 11))))]) ]));
}
