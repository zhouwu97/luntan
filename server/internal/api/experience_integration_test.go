...
func TestGuestUpgradeInPlaceWhenEmailIsNew(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT u.id, u.username, u.status, COALESCE(up.nickname, u.username),
		...
		FOR UPDATE OF u`)).WithArgs("newuser@example.com").WillReturnError(sql.ErrNoRows)
	...
	resp, err := svc.LoginByEmail(context.Background(), "newuser@example.com", "", "usr_guest123", auth.SessionMetadata{UserAgent: "ua", IPAddress: "127.0.0.1"})
	if err != nil {
...
func TestGuestLoginExistingEmailDoesNotMergeGuestExp(t *testing.T) {
	...
	resp, err := svc.LoginByEmail(context.Background(), "existing@example.com", "", "usr_guest123", auth.SessionMetadata{UserAgent: "ua", IPAddress: "127.0.0.1"})
	if err != nil {
		t.Fatalf("LoginByEmail failed: %v", err)
	}
