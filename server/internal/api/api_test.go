...
func TestCreatePostWritesRevisionAndIsIdempotentByUserKey(t *testing.T) {
	...
	defer db.Close()
	mock.ExpectQuery(`(?s)SELECT u\.id, u\.username.*FROM sessions s`).WithArgs(sqlmock.AnyArg()).WillReturnRows(sqlmock.NewRows([]string{"id", "username", "status", "nickname", "level", "experience", "account_type", "email", "email_verified", "email_verified_at"}).AddRow("u1", "user", "active", "用户", 2, 100, "email", "", false, nil))
	mock.ExpectBegin()
...
func TestMediaUploadTokenCreatesPendingAssetWithSignedURL(t *testing.T) {
	...
	defer db.Close()
	mock.ExpectQuery(`(?s)SELECT u\.id, u\.username.*FROM sessions s`).WithArgs(sqlmock.AnyArg()).WillReturnRows(sqlmock.NewRows([]string{"id", "username", "status", "nickname", "level", "experience", "account_type", "email", "email_verified", "email_verified_at"}).AddRow("u1", "user", "active", "User", 1, 0, "email", "", false, nil))
