...
func TestMigrationFilesAreOrderedAndPaired(t *testing.T) {
	...
	if len(files) != 5 || files[0].Version != "000001" || files[1].Version != "000002" || files[2].Version != "000003" || files[3].Version != "000004" || files[4].Version != "000005" {
...
func TestMigrationsAgainstPostgres(t *testing.T) {
	...
	for _, table := range []string{"users", "user_profiles", "community_categories", "communities", "posts", "user_auth_methods", "sessions", "refresh_tokens", "post_revisions", "post_idempotency_keys"} {
		var exists bool
