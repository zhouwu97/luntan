  ...
  return (
    <>
      ...
      <main className="page-frame">
        {loading ? <div className="detail-skeleton"><div /><div /></div> : profile ? <ProfileView profile={profile} posts={posts} isSelf={user?.id === profile.id} onFollow={toggleFollow} followBusy={followBusy} /> : <div className="empty-state"><span className="empty-icon"><Icon name="user" size={24} /></span><h1>个人主页不可用</h1><p>{error || "这个用户可能不存在或已被隐藏。"}</p><Link href="/" className="primary-link">返回首页</Link></div>}
