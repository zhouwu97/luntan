...
export '../domain/models.dart' show Comment, Community, CommunityCategory, FeedPage, MediaAsset, Post, PostDetail, Reaction, User, ViewerPostState, relativeTimeLabel;
...
class ForumStore extends ChangeNotifier {
  ...
  List<User> searchUsers(String query) {
    ...
    return _users.values.where((user) => '${user.nickname} ${user.username} ${user.signature ?? ''}'.toLowerCase().contains(keyword)).toList();
  }

  List<Community> searchCommunities(String query) {
    final keyword = query.trim().toLowerCase();
    if (keyword.isEmpty) return communities;
    return communities.where((community) => '${community.name} ${community.description}'.toLowerCase().contains(keyword)).toList();
  }

  List<Comment> commentsFor(Post post) => List.unmodifiable(commentsByPost[post.id] ?? const <Comment>[]);

  List<Comment> commentsByAuthor(String authorId) => commentsByPost.values.expand((items) => items).where((comment) => comment.authorId == authorId && comment.publicationStatus == CommentPublicationStatus.published).toList();

  List<Post> get bookmarkedPosts => posts.where((post) => post.isBookmarked).toList();
  ...
  void addPost(PostDraft draft) {
    ...
    final post = Post(
      ...
      type: draft.isGameShare ? PostType.gameShare : draft.isPoll ? PostType.poll : PostType.normal,
  ...
  Comment addComment(Post post, String content, {String? parentId, String? replyToUserId}) {
    final now = DateTime.now();
...
const _keyboard = [MediaAsset(id: 'media-keyboard', type: MediaType.image, url: 'https://images.unsplash.com/photo-1688525425714-12851414c3e7?auto=format&fit=crop&w=900&q=82', width: 1200, height: 900, emoji: '⌨️', label: '机械键盘', colors: [0xFFB7D9FF, 0xFF6D9CDE])];
const _campus = [MediaAsset(id: 'media-campus', type: MediaType.image, url: 'https://images.unsplash.com/photo-1743268139156-b9d8c1d4e98e?auto=format&fit=crop&w=700&q=80', width: 1200, height: 800, emoji: '📚', label: '校园桌面', colors: [0xFFCEE9FF, 0xFF7FB6E5])];
const _daily = [MediaAsset(id: 'media-daily', type: MediaType.image, url: 'https://static.wixstatic.com/media/94d226_1f3cac2a4f5b450d98d0bc6355cc30ac~mv2.jpg/v1/fill/w_1000,h_900,al_c,q_85/94d226_1f3cac2a4f5b450d98d0bc6355cc30ac~mv2.jpg', width: 1000, height: 900, emoji: '🌙', label: '晚霞随拍', colors: [0xFFFFD2B2, 0xFFEA8FB1])];
const _gameShare = [MediaAsset(id: 'media-game-share', type: MediaType.image, url: 'https://images.squarespace-cdn.com/content/v1/62fbfbed423b4f1bb8caed31/dce67434-bb6b-4a71-87f8-e98dd70cab47/v2-e242x-15y2g.jpg', width: 1000, height: 700, emoji: '🎮', label: '玩法示例', colors: [0xFFFFE5BC, 0xFFE5A55B])];
...
List<Post> _seedPosts() => [
      ...
      _post(id: 'u5', authorId: 'user-1', title: '从零搭一个舒服的宿舍桌面：完整清单', body: '把我这半年试过的桌搭方案重新整理了一遍，预算、使用感和优先级都写上。', section: ForumSection.unboxing, tag: '大型拆箱', hoursAgo: 72, comments: 236, views: 7800, extraTag: '精华', isFeatured: true, images: [..._keyboard, ..._campus, ..._daily, ..._gameShare, ..._keyboard]),
      ...
      _post(id: 'c3', authorId: 'user-7', title: '新生校园生活问答合集：宿舍 / 食堂 / 图书馆', body: '把高频问题都整理在一起，后续还能持续补充，适合固定沉淀。', section: ForumSection.community, tag: '酱紫社区', hoursAgo: 336, comments: 329, views: 9400, extraTag: '精华', isFeatured: true, images: [..._campus, ..._daily, ..._gameShare]),
      ...
      _post(id: 'd3', authorId: 'user-10', title: '玩法分享：宿舍多人桌游局怎么组？', body: '整理一套四到六人都能快速上手的桌游流程，包含分组、计分和新手友好规则，欢迎大家补充自己的玩法。', section: ForumSection.daily, tag: '玩法分享', hoursAgo: 24, comments: 35, views: 890, images: _gameShare),
...
Map<String, List<Comment>> _seedComments() => {
      'u1': [
        Comment(id: 'comment-u1-1', postId: 'u1', authorId: 'user-2', content: '这个整理很有用，尤其是宿舍噪音那段。想问下你用的是什么轴？', likeCount: 12, replyCount: 1, createdAt: _now.subtract(const Duration(hours: 1)), updatedAt: _now.subtract(const Duration(hours: 1))),
        Comment(id: 'comment-u1-1-1', postId: 'u1', authorId: 'user-1', rootId: 'comment-u1-1', parentId: 'comment-u1-1', replyToUserId: 'user-2', content: '我这把是偏轻的线性轴，之后可以单独补一楼声音和压力克数。', createdAt: _now.subtract(const Duration(minutes: 45)), updatedAt: _now.subtract(const Duration(minutes: 45))),
        Comment(id: 'comment-u1-2', postId: 'u1', authorId: 'user-3', content: '图片比例现在看着舒服多了，一张图没有占满整个屏幕。', likeCount: 7, createdAt: _now.subtract(const Duration(minutes: 48)), updatedAt: _now.subtract(const Duration(minutes: 48))),
        Comment(id: 'comment-u1-3', postId: 'u1', authorId: 'user-4', content: '建议再加一个“只看楼主”，这种长讨论会比较方便。', likeCount: 4, createdAt: _now.subtract(const Duration(minutes: 23)), updatedAt: _now.subtract(const Duration(minutes: 23))),
      ],
      'c1': [
        Comment(id: 'comment-c1-1', postId: 'c1', authorId: 'user-1', content: '二食堂的砂锅晚上去比较不排队，先收藏了。', likeCount: 8, createdAt: _now.subtract(const Duration(hours: 2)), updatedAt: _now.subtract(const Duration(hours: 2))),
      ],
    };
